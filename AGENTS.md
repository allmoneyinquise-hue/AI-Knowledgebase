...
**Primary Use Case:** Decision rubric for all new projects. When proposing any idea—building an agent, designing a system, evaluating tech, writing specs, optimizing infrastructure—Codex retrieves relevant KB sections and grounds recommendations in captured best practices. The KB is the operational foundation for sound judgment, not just a reference library.
...
→ `future-reference/playbooks/multi-agent-orchestration.md → Codex Agent Teams section`
...
Signals: "connect to [external tool]," "give the agent access to," "tool integrations," "Codex needs to call [API]," "build an MCP server," "external system access"
...
- Files here are read by Codex each session to pick up mid-investigation without losing context
